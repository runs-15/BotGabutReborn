...
class Presensi(Cog):
    def __init__(self, bot):
        ...
        #get day scheduler
        self.scheduler.add_job(self.get_day, CronTrigger(hour=16, minute=55, day_of_week="mon-fri", timezone="Asia/Jakarta"))
    ...
    async def presensi_pagi(self):
        ...
        if pause_datang > 0:
            ...
            for j in [i for i in db.servers_con['servers']['server'].find()]:
                channel = await self.bot.fetch_channel(int(j['presensi_channel']))
            ...
            for i in sequence:
                if int(i['nis']) not in [int(x['nis']) for x in db.siswa_con['siswa']['eksepsi_presensi'].find({'status' : 'datang'})]:
                    ...
                    try:
                        user = await self.bot.fetch_user(int(i['discord_id']))
    ...
    async def presensi_pm(self):
        ...
        if pause_pm > 0:
            ...
            for j in [i for i in db.servers_con['servers']['server'].find()]:
                channel = await self.bot.fecth_channel(int(j['presensi_channel']))
